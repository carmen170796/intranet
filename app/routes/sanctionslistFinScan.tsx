...
export default function SanctionslistFinnscan() {
  ...
  useEffect(() => {
    ...
    results && setNumResults(results.filter((result) => result.returned != 0).length);
  }, [formSubmitData, results]);
  ...
  return (
    <Form method="post" className='pl-40' encType='multipart/form-data'>
      ...
      {
        (results && numResults === 0) && <FormLabel className='mt-6 text-green-600 font-bold'>Keine Suchergebnisse gefunden.</FormLabel>
      }
